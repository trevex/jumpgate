...
kind-images: ## Build the container images used by the kind env
	...
	docker build $(DOCKER_BUILD_FLAGS) -f deploy/docker/rdp-proxy.Dockerfile -t jumpgate/rdp-proxy:dev .
	...
	docker build $(DOCKER_BUILD_FLAGS) -f test/env/testworkload/pg-target.Dockerfile -t jumpgate/testworkload-pg-target:dev test/env/testworkload
...
kind-redeploy: ## Rebuild app images, reload into the running cluster, helm upgrade + restart app pods (NOTE: cannot apply cluster.yaml changes like extraPortMappings — those need kind-down/kind-up)
	$(MAKE) kind-images
	kind load docker-image jumpgate/warden:dev jumpgate/gateway:dev jumpgate/ssh-proxy:dev jumpgate/pg-proxy:dev jumpgate/rdp-proxy:dev jumpgate/k8s-broker:dev jumpgate/k8s-agent:dev --name $(KIND_CLUSTER)
	...
	kubectl rollout restart deploy/jumpgate-warden deploy/jumpgate-gateway deploy/jumpgate-ssh-proxy deploy/jumpgate-pg-proxy deploy/jumpgate-rdp-proxy deploy/jumpgate-k8s-broker
	...
	kubectl rollout status deploy/jumpgate-rdp-proxy --timeout=180s
