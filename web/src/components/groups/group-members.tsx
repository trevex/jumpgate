/**
 * group-members.tsx — shared group membership body.
 *
 * The reusable membership UI for a single group: an Add-member seam (a
 * User | Group picker), a Members (users) section, and a Sub-groups (nested
 * groups) section — all cap-gated on the membership caps, with scoped
 * invalidation. This is the body only (no Sheet / panel chrome); callers
 * (the Directory Sheet, the Catalog group detail pane) supply their own frame.
 *
 ...
 *
 * The "Add member" control and every Remove are gated on the membership caps
 * (`identity:group:add-member` / `remove-member`); the server is the real gate.
 * Each mutation invalidates this group's members query (scoped via
 * createConnectQueryKey so other groups' member lists are untouched) + toasts;
...
import {
  ...
  partitionMembers,
  memberCount,
} from "@/routes/directory/group-membership";
import { AddMemberPicker } from "@/routes/directory/add-member-picker";
import {
  User as UserIcon,
  Users as UsersIcon,
  UsersRound,
...
/**
 * The minimal group identity the membership body needs: the id it queries and
 * mutates, the name for toasts/labels, and (optionally) the folder home so the
 * Add-member picker can echo it. Both the Directory (which holds a full Group)
 * and the Catalog detail pane (which only knows id + name) can satisfy this.
 */
...
export function useInvalidateMembers(groupId: string) {
...
/**
 * The reusable membership body for one group. Chrome-free: renders the
 * Add-member seam + Members / Sub-groups sections + the AddMemberPicker.
 */
export function GroupMembers({ group }: { group: GroupRef }) {
