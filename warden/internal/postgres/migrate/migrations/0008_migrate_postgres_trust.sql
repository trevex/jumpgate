...
-- Enable the PostgreSQL verification vertical slice for pre-existing assets: queue one
-- onboarding probe per existing postgres asset at its current endpoint revision, so the
...
SELECT a.id, a.endpoint_revision, 'postgres', 'queued', 'onboarding'
FROM assets a
WHERE a.kind = 'postgres'
  AND NOT EXISTS (
      SELECT 1
