...
func TestListPoliciesForAssetAndGroup(t *testing.T) {
	...
	otherAsset, err := q.CreateAsset(ctx, gen.CreateAssetParams{
	...
	pAsset, err := q.CreateRequestPolicy(ctx, gen.CreateRequestPolicyParams{
	...
	grp, err := q.CreateGroup(ctx, gen.CreateGroupParams{Name: "sre-" + uuid.NewString()[:8]})
	if err != nil {
		t.Fatalf("CreateGroup: %v", err)
	}
	otherGrp, err := q.CreateGroup(ctx, gen.CreateGroupParams{Name: "ops-" + uuid.NewString()[:8]})
	if err != nil {
