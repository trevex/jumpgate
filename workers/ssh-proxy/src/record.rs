...
pub enum RecMsg {
    /// A single recorded event to encode and append.
    Event {
        t: f64,
        kind: crate::asciicast::EventKind,
        data: Vec<u8>,
    },
...
impl RecorderHandle {
    /// Non-blocking send of an event. `Err(())` means the channel is full or
    /// closed — a fail-closed trigger for the caller. The unit error is
    ...
    pub fn try_event(
        ...
        self.tx
            .try_send(RecMsg::Event { t, kind, data })
            .map_err(|_| ())
    }
...
impl<U: PartUploader> Recorder<U> {
    ...
    async fn run(
        ...
        while let Some(msg) = rx.recv().await {
            match msg {
                RecMsg::Event { t, kind, data } => {
                    let line = crate::asciicast::event_line(t, kind, &data);
                    self.append(&line);
...
mod tests {
    use super::*;
    use crate::asciicast::{event_line, EventKind, Header};
    ...
    async fn completes_and_hashes() {
        let header = Header::new(80, 24, 1_700_000_000);
        ...
        // Independently recompute the expected bytes/hash.
        let events: Vec<(f64, EventKind, Vec<u8>)> = (0..64)
            .map(|i| {
                (
                    i as f64 * 0.1,
                    EventKind::Output,
        ...
        let mut expected = header.to_line();
        for (t, kind, data) in &events {
            expected.extend_from_slice(&event_line(*t, *kind, data));
        }
        assert!(
        ...
        let (handle, join) = spawn_recorder(ArcUploader(uploader.clone()), header.clone(), cfg);
        for (t, kind, data) in &events {
            handle.try_event(*t, *kind, data.clone()).expect("send");
        }
        handle.finish().await;
        ...
        // equals the expected byte stream.
    ...
    async fn aborts_on_uploader_error() {
        ...
        let (handle, join) = spawn_recorder(uploader, header, cfg);
        // Feed more than part_size to force a (failing) flush of part 1.
        let big = vec![b'x'; 4096];
        handle.try_event(0.0, EventKind::Output, big).expect("send");
        handle.finish().await;
        let report = join.await.expect("join");
    ...
    async fn fail_signal_aborts() {
        ...
        let (handle, join) = spawn_recorder(uploader, header, cfg);
        handle
            .try_event(0.0, EventKind::Output, b"hi".to_vec())
            .expect("send");
        handle.fail().await;
        let report = join.await.expect("join");
    ...
    async fn try_send_overflow_maps_to_err() {
        // A closed channel (receiver dropped) makes try_event fail-closed.
        let (tx, rx) = mpsc::channel::<RecMsg>(1);
        drop(rx);
        let handle = RecorderHandle { tx };
        assert_eq!(
            handle.try_event(0.0, EventKind::Output, b"x".to_vec()),
            Err(())
        );
    }
