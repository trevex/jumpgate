...
// verifyChainAndRole verifies the leaf chains to pool and carries exactly one
// SPIFFE URI SAN whose role == expectRole; it returns the parsed Identity. Fail closed.
func verifyChainAndRole(certs []*x509.Certificate, pool *x509.CertPool, expectRole string, eku x509.ExtKeyUsage) (Identity, error) {
	if len(certs) == 0 {
		return Identity{}, errors.New("peer presented no certificate")
	}
	opts := x509.VerifyOptions{Roots: pool, Intermediates: x509.NewCertPool(), KeyUsages: []x509.ExtKeyUsage{eku}}
	for _, inter := range certs[1:] {
		opts.Intermediates.AddCert(inter)
	...
	if _, err := certs[0].Verify(opts); err != nil {
		return Identity{}, fmt.Errorf("mesh chain verify: %w", err)
	}
	uris := certs[0].URIs
	if len(uris) != 1 {
		return Identity{}, fmt.Errorf("expected one URI SAN, got %v", uris)
	}
	id, err := ParseIdentity(uris[0])
	if err != nil {
		return Identity{}, err
	}
	if id.Role != expectRole {
		return Identity{}, fmt.Errorf("peer role %q, want %q", id.Role, expectRole)
	}
	return id, nil
}

// ClientTLSConfigRole is ClientTLSConfig but pins the server's ROLE (any id).
func ClientTLSConfigRole(leaf tls.Certificate, pool *x509.CertPool, expectRole string) *tls.Config {
	return &tls.Config{
		Certificates:       []tls.Certificate{leaf},
		RootCAs:            pool,
		MinVersion:         tls.VersionTLS13,
		NextProtos:         []string{"h2"},
		InsecureSkipVerify: true, //nolint:gosec // chain+role pinned in VerifyConnection
		VerifyConnection: func(cs tls.ConnectionState) error {
			_, err := verifyChainAndRole(cs.PeerCertificates, pool, expectRole, x509.ExtKeyUsageServerAuth)
			return err
		},
	}
}

// ServerTLSConfigRole requires+verifies a client cert whose role == expectRole (any id).
func ServerTLSConfigRole(leaf tls.Certificate, pool *x509.CertPool, expectRole string) *tls.Config {
	return &tls.Config{
		Certificates:           []tls.Certificate{leaf},
		ClientCAs:              pool,
		ClientAuth:             tls.RequireAndVerifyClientCert,
		...
		VerifyPeerCertificate: func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			...
			_, err := verifyChainAndRole(certs, pool, expectRole, x509.ExtKeyUsageClientAuth)
			return err
		},
	}
}

// IdentityFromConn extracts the verified peer Identity from a completed mesh
// handshake (call after Handshake / first byte). The chain was already verified
// by ServerTLSConfigRole; here we only parse the SPIFFE id.
func IdentityFromConn(cs tls.ConnectionState) (Identity, error) {
	if len(cs.PeerCertificates) == 0 {
		return Identity{}, errors.New("no peer certificate")
	}
	uris := cs.PeerCertificates[0].URIs
	if len(uris) != 1 {
		return Identity{}, fmt.Errorf("expected one URI SAN, got %v", uris)
	}
	return ParseIdentity(uris[0])
}
