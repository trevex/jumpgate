...
/// A live session's teardown handle: the bridge selects on `cancel.notified()`.
#[derive(Clone)]
...
/// Runs the WorkerStream control loop forever: connect to warden over mesh mTLS,
/// Register, heartbeat, forward SessionEnded, and dispatch Teardown to the
...
async fn connect_and_run(
    ...
    heartbeat.tick().await; // first tick fires immediately; skip it (just registered).
        ...
        tokio::select! {
            ...
            msg = inbound.message() => {
                match msg.context("read from worker_stream")? {
                    ...
                    None => return Ok(()), // server closed the stream: reconnect.
                }
            }
            ...
            ended = session_ended_rx.recv() => {
                match ended {
                    ...
                    None => return Ok(()), // data plane dropped its sender: shutting down.
                }
            }
        }
    }
