...
| Service layout (Access-Model v2) | Four ConnectRPC services: **IdentityService** (users/groups/memberships/lifecycle), **CatalogService** (folders/assets + user-facing discovery), **AccessService** (all admin authz config: roles, role_grants, role_bindings, request_policies + subjects, ExplainRole), **AccessRequestService** (JIT runtime: `ResolveApproval` now; RequestAccess/Approve/Deny/Revoke + grants + reaper are M3c) | Clean boundaries: *how access is defined* (static, admin — AccessService) vs *how it is obtained* (dynamic, user + approver — AccessRequestService); catalog no longer conflates the resource tree with the whole RBAC model. |
