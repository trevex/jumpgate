...
- Provenance and SBOM. Each published image carries a SLSA build-provenance
  attestation and an SPDX SBOM, attached to the image index by BuildKit. The
  signed image digest covers that index, so tampering with an attestation
...
- Scanned. Images and dependencies are scanned in CI: trivy covers the image and
  the Go, Rust, and pnpm dependency manifests; govulncheck adds Go reachability
  analysis; cargo-deny enforces the Rust dependency ban policy. The gate fails on
  fixable HIGH and CRITICAL findings.
...
# Inspect the SLSA build-provenance attestation (attached to the image index).
...
| Tampered or unattested image deployed | Images build from digest-pinned distroless bases and publish with a SLSA provenance attestation and SPDX SBOM (BuildKit, on the signed index); the digest is signed by cosign keyless (GitHub OIDC, Rekor); deployers verify with `cosign verify` and inspect provenance/SBOM via `docker buildx imagetools inspect` | Implemented |
