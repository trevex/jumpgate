...
Milestones **M1 (foundation)** and **M2 (access-model core)** are complete. **M3 is in progress**: M3a (hash-chained audit log, Append/Verify) and M3b (request-policy per (role, scope) — Resolver) are done. The access-model v2 refactor split authorization config into a dedicated `AccessService` and the JIT runtime into `AccessRequestService`, shrinking `CatalogService` to resources + visibility. Delivered so far: devshell, workspaces, protobuf codegen, the control-plane data layer (Postgres schema, sqlc, embedded migrations), the `Authorizer` seam with a recursive-CTE backend, graceful shutdown, ConnectRPC with `AuthService` + `IdentityService` + `CatalogService` + `AccessService` + `AccessRequestService`, admin bootstrap via env vars, expired-token GC, and green CI. See [roadmap.md](roadmap.md) for what's next.
